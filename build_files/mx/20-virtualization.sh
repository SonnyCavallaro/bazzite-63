...
# foreign-arch containers. The `qemu` metapackage is deliberately not
# requested; it used to ride in anyway as a hard dependency of
# podman-bootc (gotcha #23), which 10-container-runtime.sh replaced with
# bcvk — bcvk requires qemu-img and qemu-kvm only.
dnf5 -y install \
    libvirt \
    libvirt-nss \
    qemu-img \
