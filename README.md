...
- **Adds, never imposes.** Bazzite does the heavy lifting; each addition only smooths a rough edge or
  ...
  `bazzite-dx`](docs/upstream-divergences.md).
...
with `swtpm` for TPM 2.0 and KVM module options tuned so Windows 11 guests just boot. No `ujust
...
([the full story](docs/upstream-divergences.md))

The itemised list, with provenance and rationale for every choice, lives in
[docs/upstream-divergences.md](docs/upstream-divergences.md).
