...
Three variants, identical except for the Bazzite base they layer on — pick the one for your
GPU:
...
Each ships on two rolling streams — `:stable` and `:testing` — plus immutable dated tags (e.g.
`:44.20260511`) for pinning; `:latest` aliases stable. A watcher rebuilds within an hour of any
...
# Rebuilding after editing build_files/? Add --build-arg VERSION=verify1 (bump per retry):
# the build scripts are bind-mounted, so their content never invalidates the layer cache
# (docs/gotchas.md #36).
```
