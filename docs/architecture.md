...
targeted cleanup) with bazzite-dx's script auto-discovery (the `find … | sort` enumeration
in `build-mx.sh`; Aurora chains its scripts by hand in `Containerfile.in`).
...
└── .claude/                     # Claude Code config (tracked: settings.json,
    ├── settings.json            # commands/, hooks/; local-only: settings.local.json,
    ├── commands/preflight.md    # audits/, sessions/ — gitignored)
