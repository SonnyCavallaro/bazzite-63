...
repofile instead (`dnf5 config-manager addrepo
--from-repofile=https://packages.microsoft.com/yumrepos/vscode/config.repo`, repo id
`vscode-yum`) — and that file ships `gpgcheck=0` **and** `repo_gpgcheck=0` of its own (MEASURED
2026-09-01, fetched from the URL above). The FIXME is gone; the unverified install is not, it
just comes from the vendor now.
...
file before `install -m 0755` into `/usr/bin/gh`. Same pattern covers `glab`, `shellcheck`, and
`shfmt`.
...
`system_files/etc/pki/rpm-gpg/`; when upstream rotates a key the vendored copy goes stale and
installs start warning (or failing in strict mode).

**Us**: **1Password** (`build_files/mx/64-1password-key.sh`):
`curl -fsSL https://downloads.1password.com/linux/keys/1password.asc` at build time — every CI
build re-fetches the current key, so a rotation lands on the next build (~1h via the upstream
watcher).

**Why it matters**: zero maintenance debt for key rotation. A vendored key goes stale silently
the day the vendor rotates it; we trade that for a per-build network call to the trust anchor
(documented as part of the "third-party `.repo` is `enabled=0`" isolation invariant).
...
**Us**: `build_files/mx/60-desktop-apps.sh` ships `gparted` (~9 MiB) as a universal partition
manager. Provenance reinforced by AmyOS, which ships gparted in their DX-style list.

**Why it matters**: a daily-driver workstation needs a GUI partition tool; discovering the gap
at the moment of need means dropping to terminal `parted` or rebooting from USB.
...
1. `copr_install_isolated "pvermeer/sunshine" "sunshine"` — the same isolated-COPR pattern used
   ...
   it Sunshine falls back to a slower PipeWire portal path).
...
nothing reads. The recipe function's ported `Fix Error 503` branch carries two more (audited
2026-08-22): it writes into `$HOME/.config/environment.d/` without creating the directory
first, and its confirmation `echo` truncates the `.conf` suffix from the path it reports. A fix
belongs upstream, not here. Three deliberate deviations from upstream's function: the
...
**Why it matters**: a fresh-fork user copy-pasting the Bazzite-DX template gets a rechunkless
...
`14545fd` (msi-ec), `62f459c` (GUI), `c89057a` (acpi_ec).
...
`namjaejeon/linux-ntfs`, pinned commit `ca149db`) and `build_files/mx/72-ntfsplus.sh`,
