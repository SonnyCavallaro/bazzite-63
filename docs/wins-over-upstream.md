...
3. **Recipe override** (`system_files/usr/share/ublue-os/just/84-bazzite-virt.just`):
   ...
   line (would duplicate the RPM), the redundant kargs/libvirtd bits (done at build
   time), and the `kvm.report_ignored_msrs` karg sentinel gating `vfio-on` (no such karg
   exists here). VFIO / kvmfr / USB-hot-plug / libvirt-group blocks kept verbatim for
