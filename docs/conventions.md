...
- Cite **discovered gotchas**: the `sed` pattern in `10-container-runtime.sh` carries a
  4-line comment explaining why setopt is a no-op on addrepo files — load-bearing, because
  without it the next refactor reverts to setopt and silently breaks repo isolation.
...
via post-install scriptlet), use `thirdparty_repo_install`, which sed's the resulting file
rather than calling setopt.
