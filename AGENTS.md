...
- **GUI apps** → Flatpak default-install (`build_files/mx/68-flatpak-apps.sh`).
- The image carries Bazzite + the inherited bazzite-mx developer baseline only.

`ujust setup-dev` provisions the per-user toolchain in one command.
...
2. **Every third-party `.repo` file ships `enabled=0`**. Vendor it in
   ...
   `validate-repos.sh`, install via `dnf5 -y --enablerepo=<section> install
