...
4. **Push to `develop` FIRST, always** (owner's standing instruction, 2026-08-28): the sandbox
   run (all three flavours + integrity check, no rechunk) validates a change in ~10 min of
   wall-clock (MEASURED 2026-09-01 on ubuntu-26.04, run 33508793865: the jobs run in parallel)
...
10. **`validate-just` is a gating job inside `build-{stable,testing}.yml`** (not a standalone
...
.github/scripts/            # changelog.sh (release notes) + resolve-release-tag.sh + resolve-kernel-coords.sh (akmods carrier flavour) + check-image-integrity.sh (cold post-rechunk) + promote-release-tags.sh / verify-published-signatures.sh (per-stream gate + signature proof)
...
— so upstream's own `.N` rebuild suffixes never leak into our nomenclature. Each tag exists
both as GitHub Release and as immutable GHCR image tag (`:stable` is mutable and always points
to the latest build); the upstream tag a release was built off is recorded in the release title
...
```bash
# Iterate WIP on develop (CI: 3-flavour stable-stream build + integrity check — no push, no release)
