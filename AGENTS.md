...
.github/scripts/            # changelog.sh (release notes) + resolve-release-tag.sh + resolve-kernel-coords.sh (akmods carrier flavour)
...
push to main / dispatch / watch-upstream (workflow_call) ─►
  build-{stable,testing}.yml:
     resolve  (collision .N on gh release list + GHCR tags)
       └─► build    (reusable-build, single-image, staging push + sign)
