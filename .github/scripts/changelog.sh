...
# release: the images are already pushed and signed at this point.
kernel_of() {
  skopeo inspect --retry-times 3 --no-tags --format '{{index .Labels "ostree.linux"}}' \
    "docker://ghcr.io/${OWNER_LC}/$1@$2" 2>/dev/null || echo "unknown"
}
cat <<EOF
